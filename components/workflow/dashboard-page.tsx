...
export function DashboardPage() {
  ...
  if (loading || !payload || !derived) {
    return <div className="glass-panel-strong rounded-[30px] p-6 text-sm text-muted">{strings.loadingDashboard}</div>;
  }
  ...
  const tones = {
    accent: "bg-[color:var(--accent)]/18 text-[color:var(--accent-strong)]",
    mint: "bg-[color:var(--accent-mint)]/24 text-[color:var(--success)]",
    warm: "bg-[color:var(--accent-warm)]/28 text-[color:var(--warning)]",
    gold: "bg-[color:var(--accent-gold)]/32 text-[color:var(--warning)]",
  } as const;
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        {metrics.map((metric) => (
          <article key={metric.label} className="glass-panel-strong rounded-[26px] p-5">
            <span className={clsx("inline-flex rounded-full px-3 py-1 text-xs", tones[metric.tone])}>{metric.label}</span>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_1fr_1fr]">
        <article className="glass-panel-strong rounded-[30px] p-5">
          ...
          <div className="space-y-3">
            {derived.agentRosterDisplay.map((agent) => (
              <div key={agent.id} className="flex items-center gap-3 rounded-[22px] bg-[color:var(--surface)] px-4 py-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[color:var(--surface-contrast)] text-sm font-semibold">
                  {agent.initials}
                </div>
          ...
          <div className="mt-4 space-y-3">
            {derived.recentTasks.map((task) => (
              <button
                ...
                className="flex w-full flex-col rounded-[22px] bg-[color:var(--surface)] px-4 py-3 text-left transition hover:bg-[color:var(--surface-contrast)]"
              >
        ...
        <article className="glass-panel-strong rounded-[30px] p-5">
          ...
          <div className="mt-4 space-y-3">
            {derived.upcomingRecurringRuns.map((task) => (
              <div key={task.id} className="rounded-[22px] bg-[color:var(--surface)] px-4 py-3">
                <span className="text-sm font-medium">{task.title}</span>
